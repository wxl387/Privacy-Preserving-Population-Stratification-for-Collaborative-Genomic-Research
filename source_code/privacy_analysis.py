...
if __name__ == "__main__":
    ...
    common_ds = pd.read_csv('train.csv')
    for n_test in range(1, 3):
        population = pd.read_csv(f'test_{n_test}.csv')

        for n_pop in range(1, 3):
            n_pc_list = [10]
            for n_pc in n_pc_list:
                model = joblib.load(f'model_pc{n_pc}.pkl')
                ...
                for n_cluster in range(3, 4):
                    ...
                    print('n_test: ' + str(n_test) + ', n_pop: ' + str(n_pop) + ', n_pc: ' + str(
                    ...
                    df_precision_mean.to_csv(f'precision_test{n_test}_pc{n_pc}_pop{n_pop}_nc{n_cluster}.csv')
                    df_recall_mean.to_csv(f'recall_test{n_test}_pc{n_pc}_pop{n_pop}_nc{n_cluster}.csv')
                    df_power_mean.to_csv(f'power_test{n_test}_pc{n_pc}_pop{n_pop}_nc{n_cluster}.csv')
